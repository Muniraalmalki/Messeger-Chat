...
//        }
//
//        let safeEmail = DatabaseManger.shared(emailAddress: email)
//
...
class ChatViewController: MessagesViewController {
    ...
    private var messages = [Message]()
    private  var selfSender = Sender(photoURL: "", senderId: "1", displayName: "Munira")
    
    override func viewDidLoad() {
        super.viewDidLoad()
        //        tableView.dataSource .self
        //        tableView.delegate = self
        messages.append(Message(sender: selfSender, messageId: "1", sentDate: Date(), kind: .text("Hello from Munira")))
        messages.append(Message(sender: selfSender, messageId: "2", sentDate: Date(), kind: .text("Hello from Munira")))
